          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
            <div className="flex items-start gap-5 min-w-0">
              <button 
                ...
                className="p-2.5 mt-1 text-text-tertiary hover:text-brand hover:bg-bg-card rounded-xl shadow-sm border border-transparent hover:border-border transition-all"
              >
                <ArrowLeft size={20} />
              </button>
              <div className="min-w-0">
                <div className="flex items-center gap-3 mb-2.5">
                   <h1 className="text-fa-t2 font-fa-semibold text-text-primary leading-none m-0 truncate">{t('runs.detail.title')}</h1>
                   
                   <Tooltip title="点击复制 ID">
                      <div 
                        onClick={handleCopyId}
                        className="inline-flex items-center gap-2 px-2 py-0.5 bg-bg-page border border-border rounded-tag cursor-pointer transition-all hover:border-brand/30 hover:bg-bg-card group/idchip"
                      >
                        <span className="text-fa-t7 font-fa-semibold font-mono text-text-tertiary uppercase tracking-tighter tabular-nums">
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                   ...
                   <span className="text-fa-t6 font-fa-semibold text-text-secondary truncate">Project: {run.projectName}</span>
                   <Divider type="vertical" className="bg-border-divider h-3" />
                   <span className="text-fa-t7 font-fa-semibold font-mono text-text-tertiary tabular-nums uppercase tracking-tight">
            ...
            <div className="flex items-center gap-2.5 shrink-0">
               <Tooltip title={!liveAvailable ? "暂无活跃直播会话" : ""}>
                 <Button 
                  ...
                  className="text-fa-t6 font-fa-semibold uppercase tracking-widest h-10 px-6 flex items-center gap-2 shadow-lg"
                 >
               ...
               <Button 
                ...
                className="text-fa-t6 font-fa-semibold uppercase tracking-widest h-10 px-5 flex items-center gap-2 border-border hover:border-brand hover:text-brand bg-bg-card text-text-secondary"
               >
               ...
               <Dropdown menu={{ items: moreItems }} placement="bottomRight" trigger={['click']}>
                 <Button icon={<MoreVertical size={18} />} className="h-10 w-10 flex items-center justify-center border-border bg-bg-card text-text-tertiary" />
               </Dropdown>
            </div>
          </div>
        </div>

        {/* 2. Content Layout - Combined Evidence Chain Module */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-8 space-y-8 min-w-0">
             {/* Unified Evidence Chain Module */}
             <FACard title={t('runs.detail.timeline')} density="comfort" className="bg-bg-card border-border shadow-card overflow-visible">
                <RunTimeline run={run} />
             </FACard>
